...
char virtual_mem[1024*100]
char *virtual_free_root = virtual_mem;
...
char *kalloc()
{
  char *r; int e = splhi(); uint *victim_pte; char *virtual_page;
  if (r = mem_free)
    ...
    victim_paddr = (*victim_pte) & -0x3FF
	virtual_page = virtual_free_root + ;
	virtual_free_root = *(char *)virtual_free_root;
    memcpy(P2V + virtual_page, 
